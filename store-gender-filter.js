...
function update(){const page=document.querySelector('.sf-catalog-page');if(!page)return;const cards=[...page.querySelectorAll('.sf-card')];let visible=0;cards.forEach(card=>{const show=active==='todos'||infer(card)===active;card.classList.toggle('pilar-gender-hidden',!show);if(show)visible++});const count=page.querySelector('.sf-catalog-count');if(count)count.textContent=`${visible} reloj${visible===1?'':'es'}`;page.querySelectorAll('.pilar-gender-option').forEach(b=>{const on=b.dataset.gender===active;b.classList.toggle('active',on);b.setAttribute('aria-pressed',String(on))})}
function install(){const bar=document.querySelector('.sf-catalog-page .pilar-gender-filter');if(!bar||bar.dataset.ready)return;bar.dataset.ready='1';bar.addEventListener('click',e=>{const b=e.target.closest('.pilar-gender-option');if(!b)return;active=b.dataset.gender;update()});update()}
