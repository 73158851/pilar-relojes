...
function blocks(){if(!['/catalogo','/nuevos','/ofertas'].includes(location.pathname))return;const grid=document.querySelector('.sf-catalog-page .sf-grid');if(!grid||grid.dataset.blocks)return;grid.dataset.blocks='1';const cards=[...grid.querySelectorAll('.sf-card')];if(cards.length<=BATCH)return;cards.forEach((c,i)=>{if(i>=BATCH)c.hidden=true});let shown=BATCH;const wrap=document.createElement('div');wrap.className='sfp-more-wrap';wrap.innerHTML=`<button class="sfp-more">Ver más relojes <span>${Math.min(BATCH,cards.length-shown)}+</span></button><small>Mostrando <b>${shown}</b> de ${cards.length}</small>`;grid.after(wrap);wrap.querySelector('.sfp-more').onclick=()=>{cards.slice(shown,shown+BATCH).forEach(c=>c.hidden=false);shown=Math.min(cards.length,shown+BATCH);wrap.querySelector('small').innerHTML=`Mostrando <b>${shown}</b> de ${cards.length}`;if(shown>=cards.length)wrap.querySelector('.sfp-more').remove()}}
