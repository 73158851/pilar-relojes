...
function button(){
  ...
  b.addEventListener('click',async()=>{if(!installPrompt){showInstallHelp();return}installPrompt.prompt();await installPrompt.userChoice;installPrompt=null;b.remove();});document.body.appendChild(b);return b;
}
if(!isAdmin){window.addEventListener('load',()=>button());window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();installPrompt=e;button();});window.addEventListener('appinstalled',()=>{installPrompt=null;document.querySelector('.pilar-install')?.remove();});}
